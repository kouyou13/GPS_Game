...
let now_lng;
let game_area_range;
const goal_area_range = 10;
let random_LatLng_list = [];
let temp_list = [];
let random_meter_Lat;
let random_meter_Lng;
let goal_point_probability_list = [];
...
const DeleteTeamData = (parameter) => {
  return $.ajax({
    url: "./api/delete_team.php",
    type: "POST",
    data: parameter,
    catch: false
  });
...
const UpdateGoalData = (parameter) => {
  return $.ajax({
...
const map = (position) => {
  ...
  if (make_team_flag == true){
    ...
    // });
    const goal_point_probability_list_str = JSON.stringify(goal_point_probability_list);
    const random_LatLng_list_str = JSON.stringify(random_LatLng_list);
    parameter = {'gamecode' : gamecode, 'goal_point_probability' : goal_point_probability_list_str, 'goal_LatLng' : random_LatLng_list_str};
    UpdateGoalData(parameter).then(function(team_data){
...
const clickEvent = () => {
  ...
  result_div.appendChild(modal_back_div);
  ...
  game_info_text.innerHTML = `ゲームコード：${gamecode}<br>ゲームエリアの半径：${game_area_range}[m]<br>ゴールの数：${goal_num}<br>残りゲームセット数：${gameset_num}<br>制限時間：${limit_time}[分]`;
...
const result = (gamecode_temp, goal_point_probability, clear_flag, total_score_temp, make_team_flag_temp, gameset_num_temp) => {
  gamecode = gamecode_temp;
  total_score = total_score_temp;
  make_team_flag = make_team_flag_temp;
  gameset_num = gameset_num_temp - 1;
  // limit_time = limit_time_temp;
  ...
  GetTeamData(parameter).then(function (team_data) {
    ...
    result_div.appendChild(title_h1);
    if(clear_flag == true){ // クリア
      const random_max = 100;
      let random_score = Math.floor( Math.random() * (random_max + 1) );
      if(goal_point_probability => 0){ // 確率がプラス
        if (random_score > goal_point_probability) { // 確率外
          get_score = 10;
        }
        else{ // 確率内
          get_score = 30;
      ...
      else if(goal_point_probability < 0){ // 確率がマイナス
        random_score = -1 * random_score;
        if(random_score > goal_point_probability){ // 確率外
          get_score = 10;
        }
        else{ // 確率内
      ...
      score_text.innerHTML = `${get_score}点ゲットしました！`;
      result_div.appendChild(score_text);
  
      const total_score_h2 = document.createElement('h2');
      total_score_h2.id = 'total_score_h2';
      total_score_h2.innerHTML = `合計ポイント：${total_score}点`;
      result_div.appendChild(total_score_h2);
  
      if(gameset_num > 0){
        const next_game_btn = document.createElement('input');
        next_game_btn.id = 'next_game_btn';
        next_game_btn.type = 'submit';
        next_game_btn.value = '次へ';
        result_div.appendChild(next_game_btn);
  
        next_game_btn.onclick = () => {
          clickEvent();
        };
      }
      ...
      finish_btn.value = '終了';
      result_div.appendChild(finish_btn);
  
      finish_btn.onclick = () => {
        parameter = {'gamecode' : gamecode};
        DeleteTeamData(parameter).then(function () {
          console.log('finish');
          while(result_div.lastChild){
            result_div.removeChild(result_div.lastChild);
          }
      ...
      const score_text = document.createElement('p');
      score_text.id = 'score_text';
      score_text.innerHTML = `制限時間内にクリアできませんでした`;
      result_div.appendChild(score_text);
